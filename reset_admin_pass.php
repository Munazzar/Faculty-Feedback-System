...
<form id="msform"  method='POST' action='reset_admin_pass.php'>
  <!-- progressbar -->

  <!-- fieldsets -->
  <fieldset>
    <h2 class="fs-title">Reset Admin Password</h2>
	
  

    <h6 class="fs-subtitle">All fields are mandatory</h6>
	
	<input type="text" name="id" id="id" placeholder="Admin ID" required autofocus  onblur=''/>
    ...
    <input type="submit"  class="next action-button" value="Update" name='msform'  />
		 ...
		 <a href='admin-index.php'><input type="button"  class="next cancel-button" value="Cancel" name='msform'  >	</a>
	<?php 
	...
	if(isset($_POST['pass']))
	{
		...
		if($mail!=$property_decode['admin']['id1'][0])
		{
		$counter--;
			$_SESSION['change_counter']=$counter;
			echo "<br><span style='color:red;'>Enter Correct Admin ID, $counter chances left</span>";
			 $desc=": tried changing admin password with incorrect user id as '$mail'";
	 //$query_log="(".$query.")";
require_once('logging_sub_admin.php');
			...
			if($opass!=$property_decode['admin']['pass1'][0])
			{
				$counter--;
				$_SESSION['change_counter']=$counter;
				echo "<br><span style='color:red;'>Old password is incorrect, $counter chances left</span>";
				 $desc=": tried changing admin password with incorrect old password as '$opass1'";
	 //$query_log="(".$query.")";
require_once('logging_sub_admin.php');
			}
		else{
			...
			$desc=": changed admin password with correct user id and correct old password";
	 //$query_log="(".$query.")";
require_once('logging_sub_admin.php');
			
				$property_decode['admin']['pass1'][0]=$pass;
