...
for($i=0;$i<sizeof($col_arr);$i++)
	{
		...
		echo '<b>'.$array_names[$trimmed_col].'</b>';//for each array value is trimmed and then c is removed and then name is collected from array of names
...
while($name=(mysqli_fetch_field($results)->name))
{
	...
	if($co==0)
	{
		...
		echo '<th>'.$array_names[trim($name)].'</th>';//getting the names for the code from the array
...
Developed By Munazzar CSE 2k19
</div>
</body>
</html>
