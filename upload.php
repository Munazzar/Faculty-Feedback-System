...
//updating student section
if($_FILES["studentsection"]["name"]){
$target_dir = "./docs/";
$target_file = $target_dir . basename($_FILES["studentsection"]["name"]);
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
// Check if image file is a actual image or fake image
if(isset($_POST["submit"])) {
    $check = getimagesize($_FILES["studentsection"]["tmp_name"]);
...
// Check file size
if ($_FILES["studentsection"]["size"] > 500000) {
    echo "Sorry, your file is too large.";
    $uploadOk = 0;
}
// Allow certain file formats
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["studentsection"]["tmp_name"], $target_file)) {
        echo "The file ". basename( $_FILES["studentsection"]["name"]). " has been uploaded.";
		require_once("update_student_section.php");
		 $desc=":file uploaded to $target_file successfully";
		$desc=$desc.": years of student updated in t101";
...
//delete students feedback
if($_FILES["studentsfeedback"]["name"]){
$target_dir = "./docs/";
$target_file = $target_dir . basename($_FILES["studentsfeedback"]["name"]);
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
// Check if image file is a actual image or fake image
if(isset($_POST["submit"])) {
    $check = getimagesize($_FILES["studentsfeedback"]["tmp_name"]);
...
// Check file size
if ($_FILES["studentsfeedback"]["size"] > 500000) {
    echo "Sorry, your file is too large.";
    $uploadOk = 0;
}
// Allow certain file formats
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["studentsfeedback"]["tmp_name"], $target_file)) {
        echo "The file ". basename( $_FILES["studentsfeedback"]["name"]). " has been uploaded.";
		require_once("delete_students_feedback.php");
		 $desc=":file uploaded to $target_file successfully";
		$desc=$desc.": feedback data deleted for some students";
