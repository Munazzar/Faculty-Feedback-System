...
$faculty_unique_code=array();
 $query="select c16,c26 from t106_".$property_decode['current_database']['value'];
	if($result=mysqli_query($con,$query))
	{
		if(($rows=mysqli_num_rows($result))>0)
		{
			...
			while ($row = mysqli_fetch_assoc($result)) 
			{
				...
				//echo "$subject_id=$index <br>";
				$regular_subject_code[$index]=$subject_id;
			}
		}
	}
	//print_r($regular_subject_code);
	//oe subject name as key and oe id as value assignment
	$query="select * from t107_".$property_decode['current_database']['value'];
	if($result=mysqli_query($con,$query))
	{
		if(($rows=mysqli_num_rows($result))>0)
		{
	...
	//oe subject id as key and oe subject name as value
	$query="select * from t107_".$property_decode['current_database']['value'];
	if($result=mysqli_query($con,$query))
	{
		if(($rows=mysqli_num_rows($result))>0)
	...
	$query="select * from t111_".$property_decode['current_database']['value'];
	if($result=mysqli_query($con,$query))
	{
		if(($rows=mysqli_num_rows($result))>0)
		{
	...
	$query="select * from t108_".$property_decode['current_database']['value'];
	if($result=mysqli_query($con,$query))
	{
		if(($rows=mysqli_num_rows($result))>0)
		{
	...
	$query="select c7,c6,c55 from t102";
	if($result=mysqli_query($con,$query))
	{
		if(($rows=mysqli_num_rows($result))>0)
		{
			...
			while ($row = mysqli_fetch_assoc($result)) 
			{
				...
				$faculty_unique_code[$faculty_id]=$unique;
			}
		}
	}
